...
export function WeeklyView({ selectedDate }: WeeklyViewProps) {
  ...
  const handleToggle = async (todoId: Id<"todos">, isRecurring: boolean, date: Date) => {
    try {
      await toggleTodo({
        todoId,
        date: isRecurring ? date.getTime() : undefined,
      });
    } catch (error) {
...
interface DayCardProps {
  date: Date;
  dayName: string;
  onToggle: (todoId: Id<"todos">, isRecurring: boolean, date: Date) => void;
  isCompleted: (todo: Todo, date: Date) => boolean;
}

function DayCard({ date, dayName, onToggle, isCompleted }: DayCardProps) {
  const todos = useQuery(api.todos.getTodosForDate, { date: date.getTime() });
  
  const isToday = date.toDateString() === new Date().toDateString();
  
  return (
    <div className={`bg-white rounded-lg shadow-md border p-4 min-h-[300px] ${
            ...
            return (
              ...
              >
                <div className="flex items-start gap-2">
                  <button
                    onClick={() => onToggle(todo._id, todo.isRecurring, date)}
