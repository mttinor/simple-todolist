...
export function TodoList({ todos, selectedDate }: TodoListProps) {
  const toggleTodo = useMutation(api.todos.toggleTodo);
  const deleteTodo = useMutation(api.todos.deleteTodo);

  const handleToggle = async (todoId: Id<"todos">, isRecurring: boolean) => {
    try {
      await toggleTodo({
        todoId,
        date: isRecurring ? selectedDate.getTime() : undefined,
      });
    } catch (error) {
          ...
          <div className="space-y-3">
            {pendingTodos.map((todo) => (
              <TodoItem
                ...
                onToggle={() => handleToggle(todo._id, todo.isRecurring)}
...
function TodoItem({ todo, isCompleted, onToggle, onDelete, recurringLabel }: TodoItemProps) {
  return (
